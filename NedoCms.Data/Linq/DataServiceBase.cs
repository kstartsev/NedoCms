...
namespace NedoCms.Data.Linq
{
	...
	public abstract class DataServiceBase : IDataService, IDisposable
	{
		...
		protected DataServiceBase(Func<DataContext> createContext)
		{
		...
		{
			return _context.ExecuteCommand(command, parameters);
		}
		...
		private TEntity GetEntityById<TEntity>(TEntity source)
		{
			...
			using (var dc = _createContext())
			{
				var entities = (IQueryable<TEntity>)dc.GetTable(typeof(TEntity));
				...
				originalEntity = entities.FirstOrDefault();
			}
